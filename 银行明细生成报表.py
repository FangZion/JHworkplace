...
收入类型：客户货款 | 理财赎回 | 利息、理财 | 结汇收入 | 政府补贴、退税 | 光伏收入 | 房租水电 | 贷款融资 | 集团内调拨 | 其他零星
支出类型：供应商货款 | 薪资、社保、公积金 | 银行利息、手续费 | 承兑保证金 | 基建工程款 | 集团内调拨 | 税款 | 货代费 | 理财支出 | 结汇支出 | 其他费用
...
INCOME_CATS: Dict[str, List[str]] = {
    ...
    "政府补贴、退税": ["政府补贴", "退税"],
    "光伏收入":     ["光伏"],
    "房租水电":     ["房租", "租金", "水费", "电费", "生活用水及污水处理费"],
    "贷款融资":     ["贷款融资"],
    "集团内调拨":   ["内转", "借款"],
}
INCOME_FALLBACK = "其他零星"

# 支出分类（按备注关键字匹配）
EXPENSE_CATS: Dict[str, List[str]] = {
    "供应商货款":   ["货款", "采购"],
    "薪资、社保、公积金": ["薪资", "工资", "社保", "公积金", "强积金"],
    "银行利息、手续费": ["手续费", "利息支出"],
    "承兑保证金":     ["承兑"],
    "基建工程款":     ["工程款", "建设合同"],
    "集团内调拨":   ["内转", "借款"],
...
EXPENSE_FALLBACK = "其他费用"



def classify(tx, is_income: bool) -> Tuple[str, str]:
    """返回 (大类, 子类) — 大类用于流向分组，子类即用户自定义名"""
    ...
    if is_income:
        if tx.income_flag == 1:
            return ("经营活动", "客户货款")
        for sub, kws in INCOME_CATS.items():
            for kw in kws:
                if kw in note:
                    if sub == "利息、理财":
                        act = "投资活动"
    ...
    # 支出
    for sub, kws in EXPENSE_CATS.items():
        for kw in kws:
            if kw in note:
                if sub == "基建工程款":
                    act = "投资活动"
...
def analyze(txns: List[Transaction], exchange_rates: dict = None) -> dict:
    ...
    # TOP 交易（过滤换汇/内转/贷款融资，仅保留折RMB≥100万）
    top_in = []
    for t in txns:
        if t.income<=0 or t.transfer_flag==1 or is_forex(t.note): continue
        a,s=classify(t,True)
        if s=="贷款融资": continue
        if s in ("理财赎回","结汇收入"): continue
        if to_rmb_(t.income,t.currency,er)<1000000: continue
        top_in.append(t)
    top_in = sorted(top_in, key=lambda t: -to_rmb_(t.income,t.currency,er))[:10]

    top_ex = []
    for t in txns:
        if t.expense<=0 or t.transfer_flag==1 or is_forex(t.note): continue
        a,s=classify(t,False)
        if s in ("理财支出","结汇支出"): continue
        if to_rmb_(t.expense,t.currency,er)<1000000: continue
        top_ex.append(t)
...
def make_table(doc, headers, rows, cw=None):
    ...
    for ci,h in enumerate(headers): set_cell(t.rows[0].cells[ci],h,bold=True,size=10)
    for ri,r in enumerate(rows):
        for ci,v in enumerate(r): set_cell(t.rows[ri+1].cells[ci],v,bold=(ri==len(rows)-1),size=10)
    if cw:
        for row in t.rows:
...
def build_report(txns, month_str, output_path, stats):
    ...
    inc_order = ["客户货款","理财赎回","利息、理财","结汇收入","政府补贴、退税","光伏收入","房租水电","贷款融资","集团内调拨","其他零星"]
    exp_order = ["供应商货款","薪资、社保、公积金","银行利息、手续费","承兑保证金","基建工程款","集团内调拨","税款","货代费","理财支出","结汇支出","其他费用"]
    ...
    for ccy in ccy_order:
        ...
        tr=by_ccy[ccy].get("集团内调拨",{})
        if tr.get("count",0)>0:
            tfp=(tr["income"]+tr["expense"])/max(t2["income"]+t2["expense"],1)*100
            doc.add_paragraph(f"  集团内调拨占比 {tfp:.1f}%")
        doc.add_paragraph()
...
def export_detail_xlsx(txns, output_dir, month_str):
    ...
    for sn,inc in [("收入明细",True),("支出明细",False)]:
        ...
        hd=["日期","公司","币种","备注","金额","对方户名","内转标记","货款标记","大类","子类"]
        ...
        for t in txns:
            ...
            ws.write(r,0,t.date.strftime("%Y-%m-%d")); ws.write(r,1,t.company)
            ws.write(r,2,t.currency); ws.write(r,3,t.note)
            ws.write(r,4,round(amt,2)); ws.write(r,5,t.counterparty)
            ws.write(r,6,t.transfer_flag); ws.write(r,7,t.income_flag)
            ws.write(r,8,a); ws.write(r,9,s)
            r+=1
    wb.close()
    print(f"[完成] 明细 -> {path}")
